...
mod tests {
    use super::*;
    use crate::models::AlloyComparisonRequest;

    #[test]
    fn test_compare_alloys_basic() {
        ...
        assert!(result.reference_freq_hz > 0.0);
    }

    #[test]
    fn test_compare_alloys_empty_alloy_keys() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(2.0),
            diameter_m: Some(1.5),
        ...
        let result = compare_alloys(&req, None);
        assert!(!result.metrics.is_empty());
        assert!(result.metrics.len() >= 4);
    }

    #[test]
    fn test_compare_alloys_invalid_keys() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(2.0),
            diameter_m: Some(1.5),
            alloy_keys: vec!["invalid_alloy_xyz".to_string()],
        };
        let result = compare_alloys(&req, None);
        assert!(result.metrics.is_empty());
    }
    ...
    #[test]
    fn test_get_alloy_composition_suggestion_valid() {
        let result = get_alloy_composition_suggestion(256.0, 50.0);
        assert!(result.contains_key("cu"));
        assert!(result.contains_key("sn"));
        assert!(result.contains_key("pb"));
        assert!(result.contains_key("tolerance_cents"));
        let cu = result["cu"];
        let sn = result["sn"];
        let pb = result["pb"];
        assert!((cu + sn + pb - 1.0).abs() < 0.01);
        assert!(cu > 0.7 && cu < 0.9);
        assert!(sn > 0.1 && sn < 0.25);
    }

    #[test]
    fn test_get_alloy_composition_suggestion_strict_tolerance() {
        let result = get_alloy_composition_suggestion(440.0, 5.0);
        let tol = result["tolerance_cents"];
        assert!(tol <= 5.0);
    }

    #[test]
    fn test_compare_alloys_extreme_dimensions() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(0.1),
            diameter_m: Some(0.1),
    ...
    #[test]
    fn test_comparison_table_consistency() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(2.5),
            diameter_m: Some(1.8),
        ...
        let result = compare_alloys(&req, None);
        assert_eq!(result.metrics.len(), 2);
        assert!(result.comparison_table.len() >= 2);
        for row in &result.comparison_table {
            assert!(!row.metric.is_empty());
            assert!(!row.best_alloy.is_empty());
            assert!(!row.worst_alloy.is_empty());
        }
    }

    #[test]
    fn test_radar_chart_data_valid() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(2.0),
            diameter_m: Some(1.5),
        ...
        let result = compare_alloys(&req, None);
        assert!(!result.radar_chart_data.labels.is_empty());
        assert!(!result.radar_chart_data.datasets.is_empty());
    }

    #[test]
    fn test_metrics_have_valid_natural_frequencies() {
        let req = AlloyComparisonRequest {
            bell_id: None,
            height_m: Some(2.0),
            diameter_m: Some(1.5),
        ...
        for m in &result.metrics {
            assert!(m.natural_frequencies.len() >= 8);
            assert!(m.fundamental_hz > 0.0);
            assert!(m.overall_quality_score >= 0.0 && m.overall_quality_score <= 100.0);
        }
    }
